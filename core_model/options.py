...
class Options(object):
    ...
    def initialize(self):
        ...
        parser.add_argument('--start_epoch', default=122, type=int, metavar='N',
                            help='manual epoch number (useful on restarts)')
        parser.add_argument('--fold', default='5', type=str)
        parser.add_argument('--seed', default='42', type=int)
        parser.add_argument('--data_root', default='/root/maclab-tmp/M3D/data/DFEW', type=str)

        # numeric settings
        parser.add_argument('--workers', default=13, type=int,
    ...
    def parse(self):
        ...
        args.output_path = "/root/maclab-tmp/M3D/ICMEDFER/attribute/M3DFEL/outputs/DFEW/" + args.name + "/"
        ...
        if args.dataset == "DFEW":
            args.train_dataset = os.path.join(
                args.data_root, "/root/maclab-tmp/M3D/data/DFEW/EmoLabel_DataSplit/train(single-labeled)/set_X.csv")
            args.test_dataset = os.path.join(
                args.data_root, "/root/maclab-tmp/M3D/data/DFEW/EmoLabel_DataSplit/test(single-labeled)/set_X.csv")
        elif args.dataset == "FERV39K":
            args.train_dataset = os.path.join(
                args.data_root, "/root/maclab-tmp/M3D/data/FERV39k/4_setups/All_scenes/train_All.csv")
            args.test_dataset = os.path.join(
                args.data_root, "/root/maclab-tmp/M3D/data/FERV39k/4_setups/All_scenes/test_All.csv")
